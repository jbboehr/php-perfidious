...
| `perfidious.overflow_mode` | `0` | `PHP_INI_SYSTEM` | Sets the overflow behavior when casting Linux perf-event counters from `uint64_t` to `zend_long`. See the constants `Perfidious\OVERFLOW_*` for supported values; any other value uses the default throw policy. When set to `Perfidious\OVERFLOW_WARN`, `Handle::read()` and `Handle::readArray()` emit a warning and return `null` on overflow. |
